...
In your implementation, you will use the value of cutoff frequency to define the size, mean, and variance of the Gaussian kernel. Specifically, the kernel $$G$$ should be size $$(k, k)$$ where $$k=4\cdot\text{cutoff_frequency}+1$$, have mean $$\mu=\lfloor\frac{k}{2}\rfloor$$, standard deviation $$\sigma=\text{cutoff_frequency}$$, and values that sum to 1 (i.e. $$\sum_{ij}G_{ij}=1$$).
